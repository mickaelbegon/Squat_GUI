"""Shared domain model and simulation service for squat conditions.

This module deliberately has no dependency on the command-line interface.  It is
the common entry point used by both the GUI and the CLI when a condition must be
built from application settings or simulated.
"""
...
from .export_schema import SCHEMA_VERSION
