...
- Le point d'appui est exprimé sur le plan horizontal `y=0` avec une provenance explicite : `CoP` pour le bilan analytique, `ZMP` pour `biorbd.CalcZeroMomentPoint` ou son fallback dynamique. La base géométrique projetée va du talon aux orteils. La zone fonctionnelle exclut les 15 % postérieurs; avec wedge, sa limite postérieure est déplacée jusqu'à la projection de la cheville.
...
| F17 | Presets temporels | C6, C8 | PRESENT | Six `TemporalPreset` nommés; sélecteur dédié; contrôles indépendants conservés; `Personnalisé` détecté; `Δt=0,05 s` inchangé et 281 frames pour le preset lent de 14 s | Les valeurs sont des choix pédagogiques explicites, pas des normes biomécaniques. | Valider avec l'enseignant si les six triplets doivent être adaptés au protocole du cours. | P1 |
...
- les six presets temporels demandés sont présents et distincts : `Référence` 4/2/4 s, `Lent` 6/2/6 s, `Rapide` 2/0,5/2 s, `Sans pause` 4/0/4 s, puis les deux profils asymétriques 6/1/2 s et 2/1/6 s;
- les contrôles numériques indépendants restent disponibles; toute combinaison différente est identifiée `Personnalisé`;
- tous les presets conservent exactement `Δt=0,05 s`; le preset lent produit 281 frames sur 14,00 s;
...
- une régression F17 a été corrigée : la resimulation d'une condition sauvegardée limitait encore silencieusement les phases dynamiques à 4 s; elle accepte maintenant les presets à 6 s et le cas lent conserve ses 14,00 s;
