...
| F11 | Équilibre | C7, C12 | PRESENT | limites géométriques/fonctionnelles dans `kinematics`; marges dans `observables`; GUI/export/tests wedge | La marge postérieure de 15 % est un critère pédagogique, pas une loi clinique. | Conserver la qualification didactique. | P0 |
...
| F17 | Presets temporels | C6 | PRESENT | six presets dans `didactics.TEMPORAL_PRESETS`; contrôles numériques conservés; tests dédiés | Les scénarios publics `duration_slow/fast` ne correspondent pas exactement aux presets GUI annoncés. | Aligner ou expliciter ces scénarios. | P1 |
...
|---|---|---|
| F01 | PRESENT | scénarios 181 frames/9 s et 81 frames/4 s; test systématique du pas de 0,05 s |
